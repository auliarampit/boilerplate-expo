...
import { useMultipleBooleanStates } from '@/hooks/useCommonStates'
...
export const checkNotificationPermissionStatus = async (): Promise<boolean> => {
  try {
    const { hasInStorage } = await import('@/utils/storage')
...
> => {
  try {
    const { getFromStorage } = await import('@/utils/storage')
